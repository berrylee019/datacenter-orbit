...
# 헤더 영역
st.markdown('<p class="main-title">🌐 Datacenter Orbit Dashboard</p>', unsafe_allow_html=True)
st.markdown('<p class="sub-title">전 세계 주요 테크 기업의 데이터센터 인프라 스펙 및 관제 데이터 통합 플랫폼</p>', unsafe_allow_html=True)
...
def load_datacenter_data():
    ...
    if os.path.exists(file_path):
        ...
        # data.json이 없을 경우 테스트를 위한 더미 데이터 생성 (107개 시뮬레이션용)
        # 실제 형님 환경에서는 data.json을 정상적으로 불러옵니다.
        dummy_data = [
            {
        ...
        # 107개 맞추기 위한 반복 확장 예시 (실제 파일이 있으면 위 함수가 정상 작동합니다)
        return pd.DataFrame(dummy_data)

df = load_datacenter_data()

# 4. 상단 핵심 Metric 카드 배치 (시각적 임팩트)
col1, col2, col3, col4 = st.columns(4)

total_count = len(df)
# 고유 지역 수 계산 등 안전장치
unique_regions = df['region'].nunique() if 'region' in df.columns else 0
operational_count = len(df[df['status'] == 'Operational']) if 'status' in df.columns else 0

with col1:
    st.metric(label="📊 총 관제 시설", value=f"{total_count} 개소")
with col2:
    st.metric(label="🌍 타겟 지역", value=f"{unique_regions} 개 권역")
with col3:
    st.metric(label="⚡ 가동 중 시설", value=f"{operational_count} 개소")
with col4:
    st.metric(label="🛠️ 구축/계획 단계", value=f"{total_count - operational_count} 개소")

st.divider()

# 5. 사이드바 인터랙티브 필터 및 검색창 구성
st.sidebar.header("🔍 인프라 필터 & 검색")

# 검색어 입력
search_query = st.sidebar.text_input("데이터센터 명칭/위치 검색", placeholder="예: Canada, Meta 등")

# 지역별 필터
regions = ["전체"] + list(df['region'].unique()) if 'region' in df.columns else ["전체"]
selected_region = st.sidebar.selectbox("지역(Region) 선택", regions)

# 상태별 필터
statuses = ["전체"] + list(df['status'].unique()) if 'status' in df.columns else ["전체"]
selected_status = st.sidebar.selectbox("구축 상태 선택", statuses)

# 6. 필터 적용 로직
filtered_df = df.copy()

if search_query:
    # 이름이나 위치에 검색어가 포함된 항목 필터링
...
# 7. 메인 화면 데이터 테이블 및 상세 시각화 출력
st.sub_header(f"📋 검색 결과 ({len(filtered_df)}건)")
...
if not filtered_df.empty:
    ...
    # 선택된 항목 상세 보기 (가장 첫 번째 항목 등)
    with st.expander("📌 상세 인프라 사양 뷰어 (클릭하여 펼치기)"):
        selected_name = st.selectbox("상세 정보를 확인할 시설 선택", filtered_df['name'].tolist() if 'name' in filtered_df.columns else [])
        if selected_name:
            target_row = filtered_df[filtered_df['name'] == selected_name].iloc[0]
            st.json(target_row.to_dict())
else:
    st.warning("조건에 일치하는 데이터센터 인프라 정보가 없습니다. 검색어나 필터를 조정해 주세요.")

# 하단 푸터
st.markdown("---")
st.markdown("<p style='text-align: center; color: #94A3B8;'>Datacenter Orbit Intelligence Platform © 2026</p>", unsafe_allow_html=True)
