...
# 2026년형 Streamlit 버전은 st.query_params로 직접 딕셔너리 제어가 가능합니다.
...
if query_params.get("submit_lead") == "true" and query_params.get("email"):
    ...
    with st.spinner("🚀 GitHub Issues 인프라로 리드를 즉시 송출 중..."):
        status = create_github_issue(lead_email, lead_target)
        ...
        if status == "SUCCESS":
            ...
            st.stop()
        elif status == "SECRETS_ERROR":
            st.error("❌ 전송 실패: Streamlit Secrets가 설정되지 않았습니다. 관리자 설정을 확인해 보십시오.")
            st.stop()
        else:
            st.error(f"❌ API 통신 오류 발생. (상세 코드: {status})")
